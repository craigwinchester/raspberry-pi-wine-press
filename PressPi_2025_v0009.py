...
GPIO.add_event_detect(21, GPIO.RISING, callback=button_callback, bouncetime=200)
...
def getCurrentBar():
    while True:
        try:
            currentPSI = read_ser=ser.readline()
            currentPSI = currentPSI.decode('utf-8').rstrip()
            #printBox(currentPSI)
            currentBar = int(float(currentPSI)) * 0.0689475729
            #printBox(currentBar)
            if currentBar > 1.9:  #just in case
                emergencyStop()
            else:
                return round(currentBar, 4)

        except Exception as e:
            print(f"ERROR: {e}")  # old version = print("ERROR")
            break
...
# --------------------------Update Gauge--------------------
def update_gauge():
    bar_gauge.set_value(getCurrentBar())
...
class Pressure:
    ...
    def inflateToBar(self):
        ...
        while currentBar < self.bar and emerg_flag == 0:  # turn on pressure
            GPIO.output(pressurePinList, GPIO.LOW)
            pressure_flag = 1
            currentBar = getCurrentBar()
            printBox("Inflating: ", currentBar)
            time.sleep(1)
    ...
    def deflateToBar(self):
        ...
        while currentBar > self.bar and emerg_flag == 0:
            GPIO.output(vacuumPinList, GPIO.LOW)
            pressure_flag = 2
            currentBar = getCurrentBar()
            printBox("Deflating:", currentBar)
            time.sleep(1)
                        ...
                        for j in range(self.program[i]["cycles"]):
                           ...
                           #----assumes bag was fully deflated to start with
                            ...
                            while time.time() < t_end:
                                ...
                                pTime = pTime - 1
                                currentBar = getCurrentBar()
                                if currentBar < self.program[i]["resetPressure"]:
...
def open_editor():
    ...
    tk.Button(editor, text="Add Stage", command=add_stage).grid(row=10, column=1)
    tk.Button(editor, text="Remove Stage", command=remove_stage).grid(row=10, column=2)
    tk.Button(editor, text="Save", command=save_changes).grid(row=10, column=4)
...
bar_gauge = tk_tools.Gauge(root, height=126, width=360,min_value=-1, max_value=2, unit=' BAR', divisions=10, bg="SteelBlue3")
bar_gauge.grid(row=4, column=0, columnspan=2)
